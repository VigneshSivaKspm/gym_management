...
export async function loginAdmin(
  ...
  // ⚠️ DEVELOPMENT MODE - Allow login without role verification
  const ENABLE_AUTH = false;

  if (!ENABLE_AUTH) {
    console.log(
      "🔓 [DEV-MODE] Bypassing admin verification - dev mode enabled",
    );
  ...
  // If profile fetch failed from Firestore (403), verify admin status via backend
  if (result.user.role === "TRAINEE" && result.idToken) {
    console.log(
      "📡 [ADMIN-LOGIN] Profile role is default, verifying via backend...",
    );
    try {
      const adminStatus = await verifyAdminStatus(email, result.idToken);
      if (adminStatus) {
        result.user.role = "ADMIN";
        console.log("✅ [ADMIN-LOGIN] Verified as admin via backend");
      }
    } catch (error) {
      console.warn(
        "⚠️ [ADMIN-LOGIN] Could not verify admin status via backend:",
        error,
      );
    }
  }
...
// Helper function to verify admin status via backend
async function verifyAdminStatus(
  email: string,
  idToken: string,
): Promise<boolean> {
  // ⚠️ DEVELOPMENT MODE - Skip backend verification
  const ENABLE_BACKEND_VERIFY = false;

  if (!ENABLE_BACKEND_VERIFY) {
    console.log("🔧 [DEV-MODE] Backend verification disabled - returning true");
    return true;
  }
...
export async function loginTrainer(
  ...
  if (!ENABLE_AUTH) {
    console.log(
      "🔓 [DEV-MODE] Bypassing trainer verification - dev mode enabled",
    );
    const result = await loginUser(email, password);
    // Force trainer role for dev
...
export async function loginTrainee(
  ...
  if (!ENABLE_AUTH) {
    console.log(
      "🔓 [DEV-MODE] Bypassing trainee verification - dev mode enabled",
    );
    const result = await loginUser(email, password);
    // Force trainee role for dev
    result.user.role = "TRAINEE";
    return result;
  }
...
export function saveAuthTokens(idToken: string, refreshToken: string): void {
