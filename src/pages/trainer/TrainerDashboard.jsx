...
const TrainerDashboard = () => {
  ...
  const [selectedTraineeForSchedule, setSelectedTraineeForSchedule] =
    useState(null);
          ...
          setDashboardStats({
            ...
            avgProgress:
              traineesData.length > 0
                ? Math.min(25, 5 + traineesData.length)
                : 0,
          });
        } catch (err) {
  ...
  const handleMarkAttendance = async (traineeId, status) => {
    try {
      await trainerDashboardApi.markTraineeAttendance(traineeId, {
        status,
  ...
  const handleUnassignTrainee = async (scheduleId) => {
    if (
      window.confirm(
        "Are you sure you want to cancel this session? The trainee will be notified.",
      )
    ) {
      try {
    ...
    try {
      ...
      const progressPromises = trainees.map(async (trainee) => {
        try {
          ...
          console.error(
            `Failed to load progress for trainee ${trainee.id}:`,
            err,
          );
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all duration-200 text-xs ${
                  isActive
                    ? "text-indigo-400 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-700/50"
                    : "text-slate-500 dark:text-slate-400 hover:text-indigo-400 dark:hover:text-primary-400"
                }`}
              >
                  ...
                  >
                    {[
                      ...
                      },
                      {
                        ...
                        bgColor: "bg-sky-50 dark:bg-orange-900/20",
                      },
                    ].map((stat, idx) => {
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`w-10 h-10 rounded-lg bg-gradient-to-br ${action.color} flex items-center justify-center mx-auto mb-2 group-hover:scale-110 transition-transform shadow-md`}
                              >
                                  ...
                                  <div>
                                    ...
                                    <span
                                      className={`text-xs px-2 py-0.5 rounded-full ${
                                        trainee.fitness_level === "beginner"
                          ...
                          <div className="space-y-2">
                            {todaySchedule.length > 0 ? (
                              todaySchedule.map((slot, idx) => (
                                <div
                                  key={idx}
                                  className={`p-3 rounded-lg border-l-4 ${slot.is_available ? "bg-green-50 dark:bg-green-900/20 border-l-green-500" : "bg-sky-50 dark:bg-orange-900/20 border-l-orange-500"}`}
                                >
                                  <p className="font-semibold text-slate-900 dark:text-white text-sm">
                                    {slot.start_time} - {slot.end_time}
                                  </p>
                                  <p
                                    className={`text-xs ${slot.is_available ? "text-green-600 dark:text-green-400" : "text-sky-600 dark:text-sky-400"}`}
                                  >
                                    {slot.is_available
                                      ? "✓ Available"
                                      : "◉ Booked"}
                              ...
                              {recentActivity.length > 0 ? (
                                recentActivity
                                  ...
                                  .map((activity, index) => {
                                    ...
                                    const Icon =
                                      iconMap[activity.icon] || Users;
                                    return (
                                      <div
                                        key={index}
                                        className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-700/30 dark:hover:bg-slate-700/50 transition-colors"
                                      >
                                        <div
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-3xl font-bold text-yellow-700 dark:text-yellow-300 mt-1">
                            {
                              trainees.filter(
                                (t) => t.fitness_level === "intermediate",
                              ).length
                            }
                          </p>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
                      <div>
                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">
                          My Assigned Trainees
                        </h3>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                          Personal training clients assigned to you
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                    ...
                    {trainees.length === 0 ? (
                      <div className="text-center py-16">
                        ...
                        <p className="text-slate-500 text-sm mt-2">
                          Contact admin to get trainees assigned
                        </p>
                      </div>
                    ) : (
                            ...
                            >
                              <div className="flex items-start gap-4">
                                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-primary-500 to-secondary-500 flex items-center justify-center shadow-lg flex-shrink-0">
                                  <span className="text-white font-bold text-xl">
                                    {(trainee.name || trainee.user?.name || "U")
                                      .charAt(0)
                                      .toUpperCase()}
                                  </span>
                                </div>
                              ...
                              {selectedTraineeForSchedule === trainee.id && (
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() =>
                                      handleAssignTrainee(trainee.id)
                                    }
                              ...
                              <div className="mt-4 flex gap-2">
                                <button
                                  ...
                                  className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg transition-all font-medium text-sm ${
                                    selectedTraineeForSchedule === trainee.id
                                ...
                                <button
                                  onClick={() => {
                                    setSelectedConversation({
                                      id: trainee.user_id,
                                      name: trainee.name || trainee.user?.name,
                                      isTrainee: true,
                                    });
                                    setActiveTab("messages");
                                  }}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {assignedSchedules.map((session, idx) => (
                          ...
                          >
                            ...
                            {session.notes && (
                              <p className="mt-2 text-xs text-slate-500 bg-slate-900/50 p-2 rounded">
                                {session.notes}
                              </p>
                            )}
                            <button
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
                      <div>
                        ...
                        <p className="text-slate-400 text-sm">
                          Manage your availability (assign sessions from My
                          Trainees tab)
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                      ...
                      <div className="bg-green-900/20 rounded-xl p-4 border border-green-700">
                        ...
                        <p className="text-3xl font-bold text-green-300 mt-1">
                          {mySchedule.filter((s) => s.is_available).length}
                        </p>
                      </div>
                    ...
                    {scheduleLoading ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
                        {[
                          ...
                          const daySchedule = mySchedule.filter(
                            (s) => s.day_of_week === idx,
                          );
                          const dayAssigned = assignedSchedules.filter(
                            (s) => s.day_of_week === idx,
                          );
                          return (
                            <motion.div
                              ...
                              <div className="space-y-2 min-h-[120px]">
                                {daySchedule.length > 0 ? (
                                  daySchedule.map((slot, sIdx) => {
                                    const assigned = dayAssigned.find(
                                      (a) => a.start_time === slot.start_time,
                                    );
                                    return (
                                      <motion.div
                                        key={sIdx}
                                        whileHover={{ scale: 1.02 }}
                                        className={`p-3 rounded-lg text-xs border-l-4 cursor-pointer group ${
                                          assigned
                                      ...
                                      >
                                        ...
                                        {assigned ? (
                                          <div className="mt-1">
                                            <p className="text-purple-400 font-medium text-[10px]">
                                              👤 {assigned.trainee?.name}
                                            </p>
                                          </div>
                                        ) : (
                                          <p
                                            className={`text-[10px] mt-1 ${slot.is_available ? "text-green-400" : "text-orange-400"} font-medium`}
                                          >
                                            {slot.is_available
                                              ? "✓ Available"
                                              : "◉ Booked"}
              ...
              {activeTab === "analytics" && (
                ...
                >
                    ...
                    <DashboardCard className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-6 sm:p-8 text-white overflow-hidden relative border-0">
                      ...
                      <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-white/80 text-sm">
                              Track trainee progress, metrics, and performance
                              insights
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <RefreshCw
                              className={`w-5 h-5 ${progressLoading ? "animate-spin" : ""}`}
                            />
                          </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <motion.div
                      whileHover={{ y: -5 }}
                      className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-900/40 rounded-xl p-6 border border-blue-200 dark:border-blue-700"
                    >
                    ...
                    <motion.div
                      whileHover={{ y: -5 }}
                      className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-900/40 rounded-xl p-6 border border-green-200 dark:border-green-700"
                    >
                      <div className="flex items-center justify-between mb-2">
                    ...
                    <motion.div
                      whileHover={{ y: -5 }}
                      className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-900/40 rounded-xl p-6 border border-purple-200 dark:border-purple-700"
                    >
                      <div className="flex items-center justify-between mb-2">
                    ...
                    <motion.div
                      whileHover={{ y: -5 }}
                      className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-900/40 rounded-xl p-6 border border-orange-200 dark:border-orange-700"
                    >
                      <div className="flex items-center justify-between mb-2">
                  ...
                  <DashboardCard className="p-8">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      >
                        <RefreshCw
                          className={`w-5 h-5 text-slate-600 dark:text-slate-400 ${progressLoading ? "animate-spin" : ""}`}
                        />
                      </button>
                    </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                        <p className="font-semibold text-slate-900 dark:text-white">
                                          {trainee.name || trainee.user?.name}
                                        </p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">
                                          {trainee.email || trainee.user?.email}
                                        </p>
                                    ...
                                    {prog?.weight_progress ? (
                                      <div className="flex items-center gap-2">
                                        <span className="text-sm font-semibold text-slate-900 dark:text-white">
                                          {prog.weight_progress.current ||
                                            trainee.weight ||
                                            "--"}
                                          kg
                                        </span>
                                        <ArrowRight className="w-4 h-4 text-slate-400" />
                                        <span className="text-sm font-semibold text-indigo-400 dark:text-primary-400">
                                          {trainee.target_weight || "--"}kg
                                        </span>
                                      </div>
                                    ) : (
                          ...
                          >
                            <div className="flex items-start justify-between mb-4">
                              ...
                              <div className="text-right">
                                <p className="text-3xl font-bold text-indigo-400 dark:text-primary-400">
                                  --
                                </p>
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                  Adherence %
                        ...
                        <p className="text-3xl font-bold text-green-700 dark:text-green-300 mb-1">
                          {
                            trainees.filter(
                              (t) =>
                                progressData[t.id]?.weight_progress?.change < 0,
                            ).length
                          }
                        </p>
                    ...
                    >
                      <DashboardCard className="p-6 bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 border-yellow-200 dark:border-yellow-700">
                        ...
                        <p className="text-3xl font-bold text-yellow-700 dark:text-yellow-300 mb-1">
                          {
                            trainees.filter(
                              (t) => !progressData[t.id]?.weight_progress,
                            ).length
                          }
                        </p>
                      ...
                      <DashboardCard className="p-6 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 border-blue-200 dark:border-blue-700">
                        ...
                        <p className="text-3xl font-bold text-blue-700 dark:text-blue-300 mb-1">
                          {Object.values(progressData).reduce(
                            (sum, p) =>
                              sum +
                              (p?.progress?.compliance?.measurement_frequency ||
                                0),
                            0,
                          )}
                        </p>
                  ...
                  <DashboardCard className="overflow-hidden">
                    <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-primary-500 to-secondary-500 text-white">
                      ...
                      <p className="text-primary-100">
                        Chat with your assigned trainees and admin
                      </p>
                      {unreadCount > 0 && (
                        ...
                        {trainees.length > 0 ? (
                          trainees.map((trainee) => (
                            <button
                              ...
                              className={`w-full p-3 flex items-center gap-3 hover:bg-white transition-colors border-b border-gray-100 ${
                                selectedConversation?.id === trainee.id &&
                                selectedConversation?.isTrainee
                            ...
                            >
                              ...
                              <div className="flex-1 text-left min-w-0">
                                ...
                                <span className="text-[10px] text-green-600 bg-green-100 px-2 py-0.5 rounded-full inline-block mt-1">
                                  Trainee
                                </span>
                              </div>
                            </button>
                            ...
                            {conversations.map((conv) => (
                              <button
                                ...
                                className={`w-full p-4 flex items-center gap-3 hover:bg-white transition-colors border-b border-gray-100 ${
                                  selectedConversation?.id === conv.user_id &&
                                  !selectedConversation?.isTrainee
                                ...
                                conversationMessages.map((msg) => (
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <p
                                        className={`text-[10px] mt-1 ${msg.is_mine ? "text-primary-100" : "text-slate-400"}`}
                                      >
                                        {msg.created_at
                                          ? new Date(
                                              msg.created_at,
                          ...
                          <div className="flex-1 flex items-center justify-center text-slate-500 bg-gray-50">
                            <div className="text-center">
                              ...
                              <p className="text-sm text-slate-500">
                                Choose a trainee or admin to chat with
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        <DashboardCard className="p-6 sm:p-8 bg-slate-900 border-slate-700 shadow-xl">
                          ...
                          <div className="flex items-center gap-6 mb-8 p-6 bg-gradient-to-br from-slate-800/80 to-slate-800/40 rounded-2xl border border-slate-700/50 shadow-lg">
                            ...
                            <div className="flex-1">
                              <h4 className="font-bold text-white text-xl mb-1">
                                {profileForm.name || user?.name || "Trainer"}
                              </h4>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                              ...
                              >
                                ...
                                <option value="Yoga" className="bg-slate-800">
                                  Yoga
                                </option>
                                <option value="HIIT" className="bg-slate-800">
                                  HIIT
                                </option>
                                <option value="Cardio" className="bg-slate-800">
                                  Cardio
                                </option>
                                <option
                          ...
                          <div className="mt-6">
                            ...
                            <div className="flex justify-between mt-2">
                              <p className="text-xs text-slate-500">
                                Describe your expertise, training approach, and
                                professional background
                              </p>
                              <p
                                className={`text-xs font-medium ${(profileForm.bio?.length || 0) > 450 ? "text-yellow-400" : (profileForm.bio?.length || 0) > 400 ? "text-orange-400" : "text-slate-500"}`}
                              >
                          ...
                          <div className="flex items-center gap-3 mt-8 pt-6 border-t border-slate-700">
                            <motion.button
                              ...
                              disabled={
                                savingProfile || !profileForm.name?.trim()
                              }
                            ...
                            {[
                              {
                                id: "message_alerts",
                                label: "Messages",
                                desc: "New messages from trainees",
                                icon: MessageCircle,
                                color: "blue",
                              },
                              {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <div
                                      className={`w-10 h-10 rounded-lg bg-${pref.color}-500/10 border border-${pref.color}-500/20 flex items-center justify-center`}
                                    >
                                      <Icon
                                        className={`w-5 h-5 text-${pref.color}-400`}
                                      />
                                    </div>
                                    <div>
            ...
            <div className="sticky top-0 bg-gradient-to-r from-primary-600 to-secondary-600 text-white p-6 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold">
                  {selectedTrainee.name || selectedTrainee.user?.name}
                </h2>
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">
                          {traineeAnalytics.progress.weight_progress.initial?.toFixed(
                            1,
                          )}
                          kg
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">
                          {traineeAnalytics.progress.weight_progress.current?.toFixed(
                            1,
                          )}
                          kg
                        </p>
                      ...
                      <div className="bg-white rounded-lg p-3 flex items-center gap-2">
                        <span
                          className={`text-lg font-bold ${traineeAnalytics.progress.weight_progress.change > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {traineeAnalytics.progress.weight_progress.change > 0
                            ? "+"
                            : ""}
                          {traineeAnalytics.progress.weight_progress.change?.toFixed(
                            1,
                          )}
                          kg
                        </span>
                  ...
                  {traineeAnalytics.progress?.bodyfat_progress && (
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">
                          {traineeAnalytics.progress.bodyfat_progress.initial?.toFixed(
                            1,
                          )}
                          %
                        </p>
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-slate-900">
                          {traineeAnalytics.progress.bodyfat_progress.current?.toFixed(
                            1,
                          )}
                          %
                        </p>
                      ...
                      <div className="bg-white rounded-lg p-3 flex items-center gap-2">
                        <span
                          className={`text-lg font-bold ${traineeAnalytics.progress.bodyfat_progress.change > 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {traineeAnalytics.progress.bodyfat_progress.change > 0
                            ? "+"
                            : ""}
                          {traineeAnalytics.progress.bodyfat_progress.change?.toFixed(
                            1,
                          )}
                          %
                        </span>
                  ...
                  {traineeAnalytics.progress?.compliance && (
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-slate-900">
                          {traineeAnalytics.progress.compliance.adherence_rate?.toFixed(
                            0,
                          )}
                          %
                        </span>
                      ...
                      <div className="w-full bg-gray-300 rounded-full h-3 mt-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, traineeAnalytics.progress.compliance.adherence_rate || 0)}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  {traineeAnalytics.progress?.workouts && (
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-slate-900">
                          {traineeAnalytics.progress.workouts.total}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-600">Completed</span>
                        <span className="font-bold text-slate-900">
                          {traineeAnalytics.progress.workouts.completed}
                        </span>
                      </div>
                      <div className="text-sm text-slate-600">
                        Last Workout:{" "}
                        {traineeAnalytics.progress.workouts.last_workout
                          ? new Date(
                              traineeAnalytics.progress.workouts.last_workout,
                            ).toLocaleDateString()
                          : "Never"}
              ...
              <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {traineeAnalytics.milestones?.achievements?.map(
                    (achievement, idx) => (
                      <div
                        key={idx}
                        className={`rounded-lg p-4 flex items-start gap-3 ${achievement.achieved ? "bg-white border-2 border-yellow-400" : "bg-gray-100 opacity-50"}`}
                      >
