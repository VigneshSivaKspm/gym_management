...
const DashboardOverview = ({ dashboardData }) => {
  ...
  const systemHealth = safeData.system_health || {
    status: "Unknown",
    details: "",
  };
  ...
  const metrics = [
    {
      ...
      trend:
        newSignupsWeek > 0 ? `+${newSignupsWeek} this week` : "No new signups",
    ...
    },
    {
      ...
      trend:
        todaysRevenue > 0
          ? `₹${todaysRevenue.toLocaleString()} today`
          : "No revenue today",
  ...
  const paymentInsights = [
    {
      ...
      status: pendingPayments > 0 ? "warning" : "success",
      icon: Clock,
    },
    {
      label: "Overdue",
      value: `₹${overduePayments.toLocaleString()}`,
      status: overduePayments > 0 ? "danger" : "success",
      icon: AlertCircle,
    },
    {
      label: "Today's Revenue",
      value: `₹${todaysRevenue.toLocaleString()}`,
      status: todaysRevenue > 0 ? "success" : "info",
      icon: CheckCircle,
    },
    {
      label: "Expiring Soon",
      value: `${expiringMemberships} Members`,
      status: expiringMemberships > 0 ? "info" : "success",
      icon: Calendar,
    },
  ];
  ...
  const aiSuggestions =
    aiSuggestionsData.length > 0
      ? aiSuggestionsData.map((item) =>
          typeof item === "string"
            ? item
            : item.text || item.suggestion || item.message || "No suggestion",
        )
      : [
          ...
          <div className="space-y-4">
            {topPlans.map((plan, idx) => (
              <div key={idx} className="flex items-center gap-4">
                <div
                  className={`w-11 h-11 rounded-lg ${plan.color} flex items-center justify-center text-white font-bold text-sm`}
                >
                  #{idx + 1}
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    ...
                    {plan.renewalRate > 0 && (
                      <span className="ml-2">
                        • {(plan.renewalRate * 100).toFixed(0)}% renewal
                      </span>
                    )}
                  </div>
                  <div className="mt-2 h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min((plan.revenue / maxRevenue) * 100, 100)}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 border-t border-slate-700/50">
              <div className="flex items-center justify-between mb-2">
                ...
                <Wrench
                  className={`w-4 h-4 ${equipmentMaintenance > 0 ? "text-yellow-400" : "text-green-400"}`}
                />
              </div>
